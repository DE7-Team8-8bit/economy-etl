...
S3_KEY_FILTERED_INTEREST = "processed-data/interest_rate/interest_rate"

def upload_raw_interest_rate(s3_key: str, s3_bucket_name: str, **context):

    raw_csv = fetch_interest_rate(**context)
...
def interest_rate_transform(bucket: str, raw_key:str, filtered_key: str, **context):
    
    ti = context["ti"]
    logical_dt = ti.xcom_pull(task_ids="upload_interest_rate",key="logical_dt")
...
with DAG(
    ...
    task_upload_raw_interest_rate = PythonOperator(
        task_id="upload_interest_rate",
        python_callable=upload_raw_interest_rate,
        op_kwargs={"s3_key": S3_KEY_RAW_INTEREST, "s3_bucket_name": BUCKET_NAME}
    )

    task_upload_filtered_interest_rate = PythonOperator(
        task_id="upload_filtered_interest_rate",
        python_callable=interest_rate_transform,
        op_kwargs={
            "bucket": BUCKET_NAME,
            "raw_key": S3_KEY_RAW_INTEREST,
            "filtered_key": S3_KEY_FILTERED_INTEREST,
        },
    )
    ...
    task_upload_data_snowflake = PythonOperator(
        ...
        op_kwargs={"table_name": "interest_rate"},
    )

    
    task_upload_raw_interest_rate >> task_upload_filtered_interest_rate >> task_upload_data_snowflake